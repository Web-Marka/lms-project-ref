                            ...
                            <div class="inner">
                                ...
                                <ul class="rbt-sidebar-list-wrapper instructor-list-check">
                                    @foreach ($instructor as $ins)
                                        @php
                                            $instructorCourses = App\Models\Course::where('instructor_id')
                                                ->where('status', '1')
                                                ->count();
                                        @endphp
                                        <li class="rbt-check-group">
                                            <input id="ins-list-{{ $ins->user->id }}" type="checkbox"
                                                name="ins-list-{{ $ins->user->id }}">
                                            <label for="ins-list-{{ $ins->user->id }}"> {{ $ins->user->name }}
                                                <span class="rbt-lable count">
                                                    @if ($ins->user->courses)
                                                        {{ $ins->user->courses->count() }}
                                                    @else
                                                        0
                                                    @endif
                                                </span>
