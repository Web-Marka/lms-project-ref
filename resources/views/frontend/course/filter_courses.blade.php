                                    ...
                                    <form id="filterForm" action="{{ route('filter.courses') }}" method="GET">
                                        ...
                                        <div class="filter-select rbt-modern-select search-by-category">
                                            <select id="sortSelect" name="sortSelect" data-size="7" onchange="submitForm()">
                                                <option value="default" {{ $selectedOption == 'default' ? 'selected' : '' }}>Sırala</option>
                                                <option value="newest" {{ $selectedOption == 'newest' ? 'selected' : '' }}>Yeni</option>
                                                <option value="popular" {{ $selectedOption == 'popular' ? 'selected' : '' }}>Popüler</option>
                                                <option value="price-desc" {{ $selectedOption == 'price-desc' ? 'selected' : '' }}>Fiyat Azalan</option>
                                                <option value="price-asc" {{ $selectedOption == 'price-asc' ? 'selected' : '' }}>Fiyat Artan</option>
                                            </select>
                                        </div>
                                    </form>
                                </div>
                            ...
                            <div class="inner">
                                ...
                                <ul class="rbt-sidebar-list-wrapper categories-list-check has-show-more-inner-content">
                                    ...
                                    @foreach ($categories as $course)
                                        @php
                                            $catcount = App\Models\Course::where('category_id', $course->id)
                                                ->where('status', '1')
                                                ->count();
                                        @endphp

                                        <li class="rbt-check-group">
                                            <input id="cat-list-{{ $course->id }}" type="checkbox" name="cat-list-{{ $course->id }}"
                                                onclick="window.location='{{ url('category/'.$course->category_slug.'/'.$course->id) }}'">
                                            <label for="cat-list-{{ $course->id }}">{{ $course->category_name }}
